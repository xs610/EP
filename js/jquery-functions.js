$("document").ready(function () {
  ...
  function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν δικαιούστε το Επίδομα Παιδιού για το 2024!"
        : "We are sorry but you are not entitled to the Child Allowance for the year 2024!";
  ...
  function submitForm() {
    ...
    const titleText =
      currentLanguage === "greek"
        ? "Είστε δικαιούχος! Μπορείτε να προχωρήσετε στην αίτηση!"
        : "You are eligible! You can submit yor application application!";
    ...
    currentLanguage === "greek"
      ? $(".question-container").append(
          "<br /><br /><h5 class='answer'>Ενδέχεται να σας ζητηθούν κάποια/ ή όλα τα παρακάτω δικαιολογητικά:</h5><br />"
        )
      : $(".question-container").append(
          "<br /><br /><h5 class='answer'>You may be required to provide some or all of the following documents:</h5><br />"
        );
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd("Πρέπει να έχετε ετήσιο εισόδημα από 0 έως 15.000€")
          : skipToEnd("Your income should be between 0 and 15.0000€.");
      } else if (currentQuestion === 1 && selectedRadioButtonIndex === 3) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd(
              "Πρέπει να είτε να είστε Έλληνας πολίτης που διαμένει μόνιμα στην χώρα τα τελευταία 5 έτη, είτε ως αλλοπαδός να διαμένετε νόμιμα και μόνιμα στην χώρα τα τελευταία 12 έτη."
            )
          : skipToEnd(
              "You must be either a Greek citizen living permantly in the country for the past 5 years, or as a not Greek citizen you must live permanently and legally in Greece for the past 12 years."
            );
      } else if (currentQuestion === 2 && selectedRadioButtonIndex === 9) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd(
              "Πρέπει να ανήκετε σε κάποια από τις παραπάνω κατηγορίες."
            )
          : skipToEnd("You must belong in one of the above categories.");
      } else if (currentQuestion === 3 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
        currentLanguage === "greek"
          ? skipToEnd("Πρέπει να έχετε τουλάχιστον ένα εξαρτώμενο τέκνο.")
          : skipToEnd("You must have at least one dependent child.");
      } else {
